...
begin 
	//set priorities: Lower peripheral has highest priority.
...
begin 
	//set prioritie: Lower peripheral has lowest priority.
	for(i=0;i<NUM_OF_PERIPHERALS;i=i+1) begin 
		@(posedge pclk_i);
		paddr_i = i;
		pwrite_i = 1;
...
begin 
	...
	while(i < NUM_OF_PERIPHERALS) begin 
		...
		// Check if random_value already assigned
		...
		if (exist_f == 0) begin	
			...
			penable_i = 1;
			wait(pready_o == 1);
			i = i + 1;  // Move to next peripheral only if value was accepted
		end	
	end
	@(posedge pclk_i);
			penable_i = 0;
...
begin 
	for(i=0;i<NUM_OF_PERIPHERALS;i=i+1) begin 
		@(posedge pclk_i);
		penable_i = 1;
		pwrite_i = 0; //indicating read
		paddr_i = i;
		wait(pready_o==1);
		//we will get output data at 'prdata_o' port.
	end
	@(posedge pclk_i);
	penable_i = 0;
	paddr_i = 0;
